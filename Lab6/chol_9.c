...
chol(double *A, unsigned int n)
{
	...
	for (j = 0; j < local_size; j++) {
        ...
        Aij = A[IDX(j, j, local_size)];
        for (i = j + 1; i < local_size;){
            if (i < local_size - 8){
                A[IDX(i, j, local_size)] /= Aij;
                A[IDX(i + 1, j, local_size)] /= Aij;
                A[IDX(i + 2, j, local_size)] /= Aij;
                A[IDX(i + 3, j, local_size)] /= Aij;
                A[IDX(i + 4, j, local_size)] /= Aij;
                A[IDX(i + 5, j, local_size)] /= Aij;
                A[IDX(i + 6, j, local_size)] /= Aij;
                A[IDX(i + 7, j, local_size)] /= Aij;
                i += 8;
            } else {
                A[IDX(i, j, local_size)] /= Aij;
                i++;
