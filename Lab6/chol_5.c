		...
		for (i = j; i < local_size; i++) {
			...
			if(j>8){
				for (k = 0; k < j;) {
					if(k < j-8){
						Aij -= 
						A[IDX(i, k, local_size)] * A[IDX(j, k, local_size)]
						+ A[IDX(i, k+1, local_size)] * A[IDX(j, k+1, local_size)]
						+ A[IDX(i, k+2, local_size)] * A[IDX(j, k+2, local_size)]
						+ A[IDX(i, k+3, local_size)] * A[IDX(j, k+3, local_size)]
						+ A[IDX(i, k+4, local_size)] * A[IDX(j, k+4, local_size)]
						+ A[IDX(i, k+5, local_size)] * A[IDX(j, k+5, local_size)]
						+ A[IDX(i, k+6, local_size)] * A[IDX(j, k+6, local_size)]
						+ A[IDX(i, k+7, local_size)] * A[IDX(j, k+7, local_size)];
						k+=8;
					} else{
